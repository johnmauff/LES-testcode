     ...
     +           iys,iye,iy_s,iy_e,iz1,iz2,myid,ncpu,np)
         ...
         do k = iz1,iz2
            ...
            do i = jxs,jxe,2
               ij = ij + 1
               do j = 1,ny
                  c_in(1,j,ij) = at(j,i,k)         ! note: shifting data to device
                  c_in(2,j,ij) = at(j,i+1,k)
               enddo
            enddo
            ierr = cufftExecZ2Z(pln_cb, c_in, c_out, CUFFT_INVERSE)  ! perform backward 1D C2C ffts in y for [jxs,jxe]/2
            ij = 0
            do i = jxs,jxe,2
               ij = ij + 1
               do j = 1,ny
                  at(j,i,k)   = c_out(1,j,ij)     ! note: shifting data back to host
                  at(j,i+1,k) = c_out(2,j,ij)
               enddo
            enddo
         enddo
     ...
     +        iys,iye,iy_s,iy_e,iz1,iz2,myid,ncpu,np)
         ...
         do k = iz1,iz2
            do j = iys,iye
               ...
               enddo
            enddo
            ierr = cufftExecZ2D(pln_xb, x_out, x_out)  ! perform backward 1D C2R ffts in x-direction for [iys,iye]
            do j = iys,iye
               do i = 1,nx
                  ax(i,j,k) = x_out(i,j)               ! note: shifting data back to host
               enddo
...
! ======================================================================
     ...
     +                   iys,iye,iy_s,iy_e,iz1,iz2,myid,ncpu,np)
      ...
      ! ---- transpose into y-aligned tubes
...
!$cuf kernel do <<< *,* >>>                    ! execute these loops on the device
      ...
      ! ---- transpose back into x-aligned tubes
